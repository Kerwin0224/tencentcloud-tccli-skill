...
tccli <service> help 2>&1 | grep -oE '<[A-Za-z0-9]+>' | tr -d '<>' | grep -i "<关键词>"
# 例: tccli tke help 2>&1 | grep -oE '<[A-Za-z0-9]+>' | tr -d '<>' | grep -i nodepool
```
...
# AVAILABLE VERSIONS
#     2018-05-25  (recommended)   ← 默认走这个（271 Action，全功能）
...
| 命令 | 返回 | 能否用于计数/校验 |
|:-----|:-----|:-----------------|
| `grep -c '<[A-Za-z0-9]+>'` | **行数**（恒为 2：标题行 + ACTIONS 那一行） | ❌ 禁用 |
| `grep -oE '<[A-Za-z0-9]+>' \| wc -l` | **匹配数**（每个 Action 计一次） | ✅ 用这个 |

`grep -oE` 的原始值还包含 USAGE 行的 `<action>` 占位符（`tccli <service> <action> [--param...]`），计数时需扣去——注意占位符带尖括号，过滤也要带尖括号。**正则须含数字 `[A-Za-z0-9]`**，否则漏匹配含数字的 Action 名（如 `ForwardTKEEdgeApplicationRequestV3` 的 `V3`）：

```sh
# 正确计数某服务的真实 Action 数（扣 <action> 占位符）
tccli <service> help 2>&1 | grep -oE '<[A-Za-z0-9]+>' | grep -v '<action>' | wc -l
# TKE 默认版 → 271；TKE 2022-05-01 → 22；TCR → 124
```
...
agent 手里只有 `tccli` 命令和可达的官方文档。不同信息查不同渠道，**按信息类选最便宜可达的渠道，别一刀切用真机调用**（慢、写操作有副作用、可能被 CAM 拦截）：

| 要什么 | 查哪里 | 命令 |
|:---|:---|:---|
| 入参字段名速查 | skeleton | `tccli <svc> <Action> --generate-cli-skeleton` |
| 入参必填 + 类型 + **出参字段名** | help | `tccli <svc> <Action> help` |
| 入参**枚举值** + 描述 + 嵌套 JSON 语法 | help --detail | `tccli <svc> <Action> help --detail` |
| 出参真实值 + **错误码** | 真机调用 | `tccli <svc> <Action> --region ... --Limit 1` |
| 操作意图 + 参数语义 | 文档缓存 | `curl -s https://cloudcache.tencentcs.com/capi/refs/...` |
| 配额/价格/地域支持 | 官方文档 | 腾讯云产品文档页 |

**三大陷阱**：

1. **skeleton 的 `output` 子命令未实现**——拿不到出参结构。出参字段名改用 `help` 的 `OUTPUT PARAMETER` 段（或真机最小查询）。
2. **help 普通版给必填+类型+出参字段名，但不给枚举/描述**；`--detail` 才给枚举值 + 描述 + 嵌套 JSON 语法 + 调用示例。要枚举须加 `--detail`。
3. **错误码无静态获取途径**——skeleton/help/文档缓存都不含 Action 调用失败的错误码（如 `InvalidParameter.Param`），**只能真机调用捕获** `Error.Code`。exit 0 ≠ 成功，必须解析响应 JSON 中的 `Error.Code`。

> **双源校验**（核心原则）：文档缓存提供操作意图和参数语义，但接口名/字段名/枚举/必填以 tccli 自身为准——`help`/`help --detail` 是 Action 名与入参契约的权威，真机响应是字段名与错误码的权威。调用前交叉校验两者。
...
**输出格式**：tccli 默认剥离 `Response` 包装层，顶层直接是业务字段 + `RequestId`。示例：
...
首次调用任何不熟悉的 Action 时，**先自举学习 API 合约，再走压缩管道**。按信息类选最便宜渠道（详见 §1.4 真源渠道表），四步闭环：

```
1. help --detail                    # 学入参契约：必填+类型+枚举+描述+嵌套 JSON 语法（比 skeleton 全）
2. help                             # 学出参字段名：看 OUTPUT PARAMETER 段（skeleton 的 output 未实现）
3. 最小查询 --Limit 1 --output json  # 学出参真实结构：响应字段名、嵌套层级、真实值
4. 后续调用走压缩管道                 # --filter（用上一步确认的字段名）+ --output text，省 token
```

**完成准则**：入参契约（必填+枚举）已从 `help --detail` 确认、出参字段名已从 `help` 的 OUTPUT PARAMETER 段 + 真实响应双重确认（非猜测）、`--filter` 表达式引用的字段名与响应键名逐一对应。

> ⚠️ `--filter` 字段名必须匹配 API 实际响应键名（响应是 `NodePoolSet` 就不能写 `NodePools`）。`skeleton` 只给入参字段名+嵌套骨架（类型降级为 `"String"`，无必填/枚举/描述），速查可用但不如 `help --detail` 全。
...
调用失败时，tccli 返回包含 `Error` 字段的 JSON（同样剥离 `Response` 包装层，顶层即 `Error` + `RequestId`）：
...
> 错误码无静态获取途径（skeleton/help/文档缓存都不含），**只能真机调用捕获** `Error.Code`（见 §1.4 陷阱 3）。下表是常见错误码及其处理，实际遇到的码以真机返回为准。
